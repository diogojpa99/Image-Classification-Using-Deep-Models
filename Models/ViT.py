...
class VisionTransformer(nn.Module):
    ...
    def __init__(
        ...
        self.pos_embed_flag = pos_encoding # If false, no positional encoding is used
        
        if self.pos_embed_flag:
            self.pos_embed = nn.Parameter(torch.randn(1, embed_len, embed_dim) * .02)
        else:
            self.pos_embed = torch.zeros((1, embed_len, embed_dim), requires_grad=False)
    ...
    def _pos_embed(self, x):
        ...
        if not self.pos_embed_flag:
            self.pos_embed = torch.zeros((1, self.num_tokens, self.embed_dim), requires_grad=False)
    ...
    def forward_features(self, x):
        ...
        x = self._pos_embed(x)
        if not self.pos_embed_flag:
            print(f"Pos. Embedding tensor is all zeros: {torch.all(self.pos_embed == 0)}") 
        x = self.patch_drop(x)
        x = self.norm_pre(x)
